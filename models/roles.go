...
type Role struct {
	...
	RoleName string `boil:"role_name" json:"roleName" toml:"roleName" yaml:"roleName"`
...
var RoleWhere = struct {
	...
	ID:       whereHelperuint{field: "`roles`.`id`"},
...
func (r *roleR) GetAccounts() AccountSlice {
	if r == nil {
		return nil
	}
	return r.Accounts
...
var (
	roleAllColumns            = []string{"id", "role_name"}
	roleColumnsWithoutDefault = []string{"role_name"}
...
func (o *Role) Accounts(mods ...qm.QueryMod) accountQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`accounts`.`role_id`=?", o.ID),
	)
...
func (roleL) LoadAccounts(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRole interface{}, mods queries.Applicator) error {
	var slice []*Role
	var object *Role
	...
	query := NewQuery(
		qm.From(`accounts`),
		qm.WhereIn(`accounts.role_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *Role) AddAccounts(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Account) error {
	var err error
...
func FindRole(ctx context.Context, exec boil.ContextExecutor, iD uint, selectCols ...string) (*Role, error) {
	roleObj := &Role{}
...
func RoleExists(ctx context.Context, exec boil.ContextExecutor, iD uint) (bool, error) {
	var exists bool
