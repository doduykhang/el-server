...
type Word struct {
	...
	Pronouciation string `boil:"pronouciation" json:"pronouciation" toml:"pronouciation" yaml:"pronouciation"`
...
var WordTableColumns = struct {
	...
	Pronouciation: "words.pronouciation",
	Type:          "words.type",
...
var WordWhere = struct {
	...
	Pronouciation whereHelperstring
	Type          whereHelperstring
	ManagerID     whereHelperint
}{
	ID:            whereHelperint{field: "\"words\".\"id\""},
	...
	Pronouciation: whereHelperstring{field: "\"words\".\"pronouciation\""},
	Type:          whereHelperstring{field: "\"words\".\"type\""},
	ManagerID:     whereHelperint{field: "\"words\".\"manager_id\""},
}
...
var WordRels = struct {
	Manager           string
	Users             string
	FolderUserFolders string
	Lessons           string
}{
	Manager:           "Manager",
	Users:             "Users",
	FolderUserFolders: "FolderUserFolders",
	Lessons:           "Lessons",
}
...
type wordR struct {
	...
	FolderUserFolders UserFolderSlice `boil:"FolderUserFolders" json:"FolderUserFolders" toml:"FolderUserFolders" yaml:"FolderUserFolders"`
...
func (r *wordR) GetFolderUserFolders() UserFolderSlice {
	if r == nil {
		return nil
	}
	return r.FolderUserFolders
}
...
var (
	wordAllColumns            = []string{"id", "word", "definition", "example", "pronouciation", "type", "manager_id"}
	wordColumnsWithoutDefault = []string{"word", "definition", "example", "pronouciation", "type", "manager_id"}
...
func (o *Word) Users(mods ...qm.QueryMod) userQuery {
	...
	queryMods = append(queryMods,
		qm.InnerJoin("\"user_words\" on \"users\".\"id\" = \"user_words\".\"user_id\""),
		qm.Where("\"user_words\".\"word_id\"=?", o.ID),
	)

	return Users(queryMods...)
}

// FolderUserFolders retrieves all the user_folder's UserFolders with an executor via id column.
func (o *Word) FolderUserFolders(mods ...qm.QueryMod) userFolderQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.InnerJoin("\"word_in_folder\" on \"user_folder\".\"id\" = \"word_in_folder\".\"folder_id\""),
		qm.Where("\"word_in_folder\".\"word_id\"=?", o.ID),
	)

	return UserFolders(queryMods...)
}
...
func (wordL) LoadManager(ctx context.Context, e boil.ContextExecutor, singular bool, maybeWord interface{}, mods queries.Applicator) error {
	...
	if singular {
		object = maybeWord.(*Word)
	} else {
		slice = *maybeWord.(*[]*Word)
	}
...
func (wordL) LoadUsers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeWord interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.Select("\"users\".\"id\", \"users\".\"last_name\", \"users\".\"first_name\", \"users\".\"gender\", \"users\".\"date_of_birth\", \"users\".\"account_id\", \"a\".\"word_id\""),
		qm.From("\"users\""),
		qm.InnerJoin("\"user_words\" as \"a\" on \"users\".\"id\" = \"a\".\"user_id\""),
	...
	for results.Next() {
		...
		err = results.Scan(&one.ID, &one.LastName, &one.FirstName, &one.Gender, &one.DateOfBirth, &one.AccountID, &localJoinCol)
		if err != nil {
...
// LoadFolderUserFolders allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (wordL) LoadFolderUserFolders(ctx context.Context, e boil.ContextExecutor, singular bool, maybeWord interface{}, mods queries.Applicator) error {
	var slice []*Word
	var object *Word
	...
	query := NewQuery(
		qm.Select("\"user_folder\".\"id\", \"user_folder\".\"name\", \"user_folder\".\"user_id\", \"a\".\"word_id\""),
		qm.From("\"user_folder\""),
		qm.InnerJoin("\"word_in_folder\" as \"a\" on \"user_folder\".\"id\" = \"a\".\"folder_id\""),
		qm.WhereIn("\"a\".\"word_id\" in ?", args...),
	)
	if mods != nil {
		mods.Apply(query)
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load user_folder")
	}

	var resultSlice []*UserFolder
	...
	for results.Next() {
		...
		if err != nil {
			return errors.Wrap(err, "failed to scan eager loaded results for user_folder")
		}
		if err = results.Err(); err != nil {
			return errors.Wrap(err, "failed to plebian-bind eager loaded slice user_folder")
		}
	...
	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on user_folder")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for user_folder")
	}
	...
	if singular {
		object.R.FolderUserFolders = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &userFolderR{}
			}
			foreign.R.Words = append(foreign.R.Words, object)
		}
		return nil
	...
	for i, foreign := range resultSlice {
		...
		for _, local := range slice {
			if local.ID == localJoinCol {
				local.R.FolderUserFolders = append(local.R.FolderUserFolders, foreign)
				if foreign.R == nil {
					foreign.R = &userFolderR{}
				}
				foreign.R.Words = append(foreign.R.Words, local)
...
func (wordL) LoadLessons(ctx context.Context, e boil.ContextExecutor, singular bool, maybeWord interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.Select("\"lessons\".\"id\", \"lessons\".\"name\", \"lessons\".\"description\", \"lessons\".\"manager_id\", \"a\".\"word_id\""),
		qm.From("\"lessons\""),
	...
	for results.Next() {
		...
		err = results.Scan(&one.ID, &one.Name, &one.Description, &one.ManagerID, &localJoinCol)
		if err != nil {
...
func (o *Word) AddUsers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*User) error {
	...
	for _, rel := range related {
		query := "insert into \"user_words\" (\"word_id\", \"user_id\") values ($1, $2)"
...
func (o *Word) SetUsers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*User) error {
	query := "delete from \"user_words\" where \"word_id\" = $1"
...
func (o *Word) RemoveUsers(ctx context.Context, exec boil.ContextExecutor, related ...*User) error {
	...
	var err error
	query := fmt.Sprintf(
		"delete from \"user_words\" where \"word_id\" = $1 and \"user_id\" in (%s)",
...
// AddFolderUserFolders adds the given related objects to the existing relationships
// of the word, optionally inserting them as new records.
// Appends related to o.R.FolderUserFolders.
// Sets related.R.Words appropriately.
func (o *Word) AddFolderUserFolders(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*UserFolder) error {
	var err error
	for _, rel := range related {
		if insert {
			if err = rel.Insert(ctx, exec, boil.Infer()); err != nil {
	...
	for _, rel := range related {
		query := "insert into \"word_in_folder\" (\"word_id\", \"folder_id\") values ($1, $2)"
		values := []interface{}{o.ID, rel.ID}
	...
	if o.R == nil {
		o.R = &wordR{
			FolderUserFolders: related,
		}
	} else {
		o.R.FolderUserFolders = append(o.R.FolderUserFolders, related...)
	}
...
// SetFolderUserFolders removes all previously related items of the
// word replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Words's FolderUserFolders accordingly.
// Replaces o.R.FolderUserFolders with related.
// Sets related.R.Words's FolderUserFolders accordingly.
func (o *Word) SetFolderUserFolders(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*UserFolder) error {
	query := "delete from \"word_in_folder\" where \"word_id\" = $1"
	values := []interface{}{o.ID}
	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, query)
	...
	removeFolderUserFoldersFromWordsSlice(o, related)
	if o.R != nil {
		o.R.FolderUserFolders = nil
	}

	return o.AddFolderUserFolders(ctx, exec, insert, related...)
}

// RemoveFolderUserFolders relationships from objects passed in.
// Removes related items from R.FolderUserFolders (uses pointer comparison, removal does not keep order)
// Sets related.R.Words.
func (o *Word) RemoveFolderUserFolders(ctx context.Context, exec boil.ContextExecutor, related ...*UserFolder) error {
	if len(related) == 0 {
		return nil
	}
	...
	var err error
	query := fmt.Sprintf(
		"delete from \"word_in_folder\" where \"word_id\" = $1 and \"folder_id\" in (%s)",
		strmangle.Placeholders(dialect.UseIndexPlaceholders, len(related), 2, 1),
	)
	values := []interface{}{o.ID}
	for _, rel := range related {
	...
	for _, rel := range related {
		for i, ri := range o.R.FolderUserFolders {
			if rel != ri {
				continue
			}

			ln := len(o.R.FolderUserFolders)
			if ln > 1 && i < ln-1 {
				o.R.FolderUserFolders[i] = o.R.FolderUserFolders[ln-1]
			}
			o.R.FolderUserFolders = o.R.FolderUserFolders[:ln-1]
			break
		}
	}
...
func removeFolderUserFoldersFromWordsSlice(o *Word, related []*UserFolder) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
...
func (o *Word) AddLessons(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Lesson) error {
	...
	for _, rel := range related {
		query := "insert into \"words_in_lessons\" (\"word_id\", \"lesson_id\") values ($1, $2)"
		values := []interface{}{o.ID, rel.ID}
...
func (o *Word) SetLessons(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Lesson) error {
	query := "delete from \"words_in_lessons\" where \"word_id\" = $1"
	values := []interface{}{o.ID}
	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, query)
...
func (o *Word) RemoveLessons(ctx context.Context, exec boil.ContextExecutor, related ...*Lesson) error {
	...
	var err error
	query := fmt.Sprintf(
		"delete from \"words_in_lessons\" where \"word_id\" = $1 and \"lesson_id\" in (%s)",
		strmangle.Placeholders(dialect.UseIndexPlaceholders, len(related), 2, 1),
	)
	values := []interface{}{o.ID}
	for _, rel := range related {
...
func FindWord(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*Word, error) {
	wordObj := &Word{}
	...
	query := fmt.Sprintf(
		"select %s from \"words\" where \"id\"=$1", sel,
	)
...
func (o *Word) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO \"words\" %sDEFAULT VALUES%s"
		}
...
func (o *Word) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"words\" SET %s WHERE %s",
			...
			strmangle.WhereClause("\"", "\"", len(wl)+1, wordPrimaryKeyColumns),
		)
...
func (o WordSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"words\" SET %s WHERE %s",
		...
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, wordPrimaryKeyColumns, len(o)))
...
func (o *Word) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 {
			conflict = make([]string, len(wordPrimaryKeyColumns))
			copy(conflict, wordPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"words\"", updateOnConflict, ret, update, conflict, insert)
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert words")
	}
...
func (o WordSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"words\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, wordPrimaryKeyColumns, len(o))
...
func WordExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"words\" where \"id\"=$1 limit 1)"
