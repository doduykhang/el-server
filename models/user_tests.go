...
var UserTestRels = struct {
	User                string
	Test                string
	TestUserTestDetails string
}{
	User:                "User",
	Test:                "Test",
	TestUserTestDetails: "TestUserTestDetails",
}
...
type userTestR struct {
	...
	TestUserTestDetails UserTestDetailSlice `boil:"TestUserTestDetails" json:"TestUserTestDetails" toml:"TestUserTestDetails" yaml:"TestUserTestDetails"`
}
...
func (r *userTestR) GetTestUserTestDetails() UserTestDetailSlice {
	if r == nil {
		return nil
	}
	return r.TestUserTestDetails
}
...
// TestUserTestDetails retrieves all the user_test_detail's UserTestDetails with an executor via test_id column.
func (o *UserTest) TestUserTestDetails(mods ...qm.QueryMod) userTestDetailQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`user_test_details`.`test_id`=?", o.ID),
	)
...
func (userTestL) LoadUser(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUserTest interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeUserTest.(*UserTest)
		if !ok {
			object = new(UserTest)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeUserTest)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeUserTest))
			}
		}
	} else {
		s, ok := maybeUserTest.(*[]*UserTest)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeUserTest)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeUserTest))
			}
		}
	}
...
// LoadTestUserTestDetails allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userTestL) LoadTestUserTestDetails(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUserTest interface{}, mods queries.Applicator) error {
	var slice []*UserTest
	var object *UserTest
	...
	query := NewQuery(
		qm.From(`user_test_details`),
		qm.WhereIn(`user_test_details.test_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load user_test_details")
	}

	var resultSlice []*UserTestDetail
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice user_test_details")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on user_test_details")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for user_test_details")
	}
	...
	if singular {
		object.R.TestUserTestDetails = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.TestID {
				local.R.TestUserTestDetails = append(local.R.TestUserTestDetails, foreign)
				if foreign.R == nil {
					foreign.R = &userTestDetailR{}
				}
...
// AddTestUserTestDetails adds the given related objects to the existing relationships
// of the user_test, optionally inserting them as new records.
// Appends related to o.R.TestUserTestDetails.
// Sets related.R.Test appropriately.
func (o *UserTest) AddTestUserTestDetails(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*UserTestDetail) error {
	var err error
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.ID, rel.TestID, rel.QuestionID}
	...
	if o.R == nil {
		o.R = &userTestR{
			TestUserTestDetails: related,
		}
	} else {
		o.R.TestUserTestDetails = append(o.R.TestUserTestDetails, related...)
	}
...
func FindUserTest(ctx context.Context, exec boil.ContextExecutor, iD uint, selectCols ...string) (*UserTest, error) {
	...
	query := fmt.Sprintf(
		"select %s from `user_tests` where `id`=?", sel,
	)
...
func (o *UserTest) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `user_tests` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `user_tests` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, userTestPrimaryKeyColumns))
		}
	...
	o.ID = uint(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == userTestMapping["id"] {
		goto CacheNoHooks
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for user_tests")
	}
...
func (o UserTestSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE `user_tests` SET %s WHERE %s",
		...
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, userTestPrimaryKeyColumns, len(o)))
...
var mySQLUserTestUniqueColumns = []string{
	"id",
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *UserTest) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	nzDefaults := queries.NonZeroDefaultSet(userTestColumnsWithDefault, o)
	nzUniques := queries.NonZeroDefaultSet(mySQLUserTestUniqueColumns, o)
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "`user_tests`", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `user_tests` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for user_tests")
	}
	...
	uniqueMap, err = queries.BindMapping(userTestType, userTestMapping, nzUniques)
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for user_tests")
	}
...
func UserTestExists(ctx context.Context, exec boil.ContextExecutor, iD uint) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `user_tests` where `id`=? limit 1)"
