...
func (o *Question) UserTestDetails(mods ...qm.QueryMod) userTestDetailQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`user_test_details`.`question_id`=?", o.ID),
	)
...
func (questionL) LoadUserTestDetails(ctx context.Context, e boil.ContextExecutor, singular bool, maybeQuestion interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`user_test_details.question_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func FindQuestion(ctx context.Context, exec boil.ContextExecutor, iD uint, selectCols ...string) (*Question, error) {
...
func (o *Question) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `questions` () VALUES ()%s%s"
		}
	...
	o.ID = uint(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == questionMapping["id"] {
		goto CacheNoHooks
	}
...
func (o *Question) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "`questions`", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `questions` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for questions")
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for questions")
	}
...
func QuestionExists(ctx context.Context, exec boil.ContextExecutor, iD uint) (bool, error) {
	var exists bool
