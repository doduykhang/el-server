...
type Manager struct {
	...
	Addess      string    `boil:"addess" json:"addess" toml:"addess" yaml:"addess"`
...
var ManagerWhere = struct {
	...
	Addess      whereHelperstring
	PhoneNumber whereHelperstring
	AccountID   whereHelperuint
}{
	ID:          whereHelperuint{field: "`managers`.`id`"},
	...
	Addess:      whereHelperstring{field: "`managers`.`addess`"},
	PhoneNumber: whereHelperstring{field: "`managers`.`phoneNumber`"},
	AccountID:   whereHelperuint{field: "`managers`.`account_id`"},
}
...
var (
	managerAllColumns            = []string{"id", "first_name", "last_name", "gender", "date_of_birth", "addess", "phoneNumber", "account_id"}
	managerColumnsWithoutDefault = []string{"first_name", "last_name", "gender", "addess", "phoneNumber", "account_id"}
	managerColumnsWithDefault    = []string{"id", "date_of_birth"}
...
func FindManager(ctx context.Context, exec boil.ContextExecutor, iD uint, selectCols ...string) (*Manager, error) {
	managerObj := &Manager{}
	...
	query := fmt.Sprintf(
		"select %s from `managers` where `id`=?", sel,
	)
...
func (o *Manager) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `managers` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `managers` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, managerPrimaryKeyColumns))
		}
	...
	o.ID = uint(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == managerMapping["id"] {
		goto CacheNoHooks
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for managers")
	}
...
func (o ManagerSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE `managers` SET %s WHERE %s",
		...
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, managerPrimaryKeyColumns, len(o)))
...
var mySQLManagerUniqueColumns = []string{
	"id",
	"account_id",
}
...
func (o *Manager) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	nzDefaults := queries.NonZeroDefaultSet(managerColumnsWithDefault, o)
	nzUniques := queries.NonZeroDefaultSet(mySQLManagerUniqueColumns, o)
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "`managers`", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `managers` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for managers")
	}
	...
	uniqueMap, err = queries.BindMapping(managerType, managerMapping, nzUniques)
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for managers")
	}
...
func ManagerExists(ctx context.Context, exec boil.ContextExecutor, iD uint) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `managers` where `id`=? limit 1)"
